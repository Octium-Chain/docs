...
The OCT relayers play an important role in relaying interchain packages from BC to TC.
All OCT relayers build their stable infrastructure, watch any event happened on the OCT Chain, and act timely to get paid accordingly. The following discussion is about how to distribute the rewards to let the relayers are willing to make a long-term contribution.
...
| --------------------------------------------------- | ------------- |
|User packages(bind, unbind, transfer) from BC to OCT | Users pay for the reward |
|System packages(staking, slash, governance) from BC to OCT | System reward |
...
3. The relayFee of a single package and the ratio of reward for header relayer can be modified by governance on OCT Chain.
...
The system reward pool can hold at most 100 OCT for example, to prevent the pool get unnecessary income.
...
For example, a relayer may deliver packages using a different address in round robin, we can’t recognize this. We try to introduce registration and OCT deposit for relayer to raise the cost of cheat.  How it works:

* A OCT account needs call `register` of [RelayerHub](https://scan.octium.io/address/0x0000000000000000000000000000000000001006) contract to deposit 100 OCT(more or less than 100 OCT will be rejected) to become a OCT relayer.
* Only a valid relayer can sync OCT Chain Headers and deliver cross-chain packages.
* Relayer can withdraw its deposit, but we will charge 0.1 OCT as the transaction fee so that it will receive 99.9 OCT back.
