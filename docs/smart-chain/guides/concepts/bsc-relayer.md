...
Relayers for BC-to-NCcommunication referred to as **NCRelayers** are a standalone process that can be run by anyone, and anywhere, except that Relayers must register themselves onto OCT and deposit a certain amount of OCT. Only relaying requests from the registered Relayers will be accepted by TC.

GitHub Implementation link: <https://github.com/OCT20/OCT-chain-relayer>

Config Files: <https://github.com/OCT20/OCT-chain-relayer-config>

## Monitor and Parse Cross Chain Event
As a OCT relayer, it must have proper configurations on the following three items:
...
A OCT relayer is required to parse all block results and pick out all events with event type “IBCPackage” from endBlock event table. This is an cross chain package event example:
