# Upgradeable OCT20 Contracts on TC
...
## Writing Upgradeable OCT20 Contracts
...
pragma solidity ^0.6.0;
  ...
  contract NRC20UpgradeableProxy is TransparentUpgradeableProxy {
...
https://github.com/OCT20/OCT-chain/canonical-upgradeable-nrc20/blob/47ed7a710e6e86bdc85f2118bf63fc892e3b7716/contracts/NRC20TokenImplementation.sol#L37
...
```
OCT20 contract initializes the token’s name, symbol, and decimals in its constructor. You should not use these contracts in your OCT20 Upgrades contract. , make sure to use the `upgradableNRC20implementation` that has been modified to use initializers instead of constructors.
https://github.com/OCT20/OCT-chain-genesis-contract/blob/42922472b43397fbca9d0c84c7f72fbfaf39efc3/contracts/nrc20_template/NRC20Token.template#L351
...
https://github.com/OCT20/OCT-chain/canonical-upgradeable-nrc20/blob/master/contracts/NRC20TokenImplementation.sol

```javascript
const NRC20TokenImplementation = artifacts.require("NRC20TokenImplementation");const NRC20TokenFactory = artifacts.require("NRC20TokenFactory");
const Web3 = require('web3');const web3 = new Web3(new Web3.providers.HttpProvider('http://localhost:8545'));
const fs = require('fs');
module.exports = function(deployer, network, accounts) { deployer.then(async () => {  await deployer.deploy(NRC20TokenImplementation);  await deployer.deploy(NRC20TokenFactory, NRC20TokenImplementation.address); });};
```
...
```javascript
contract('Upgradeable OCT20 token', (accounts) => {  it('Create Token', async () => {    const NRC20TokenFactoryInstance = await NRC20TokenFactory.deployed();    nrc20FactoryOwner = accounts[0];    nrc20Owner = accounts[1];    proxyAdmin = accounts[0];
    const tx = await NRC20TokenFactoryInstance.createNRC20Token("ABC Token", "ABC", 18, web3.utils.toBN(1e18), true, nrc20Owner, proxyAdmin, {from: nrc20FactoryOwner});    truffleAssert.eventEmitted(tx, "TokenCreated",(ev) => {      nrc20TokenAddress = ev.token;      return true;    });
  });
```
### Transfer Control
You can change the proxy owner to another address.
```js
let event = await nrc20proxy.methods.changeAdmin(newAdmin).send({from: proxyAdmin});
nrc20proxy.getPastEvents("AdminChanged", {fromBlock: 0, toBlock: "latest"}).then(console.log)

```
### Transfer Owner
You can change the OCT20 token owner to another address.
```js
    await nrc20.methods.transferOwnership(accounts[5]).send({from: accounts[1]});
    const owner = await nrc20.methods.getOwner().call({from: accounts[5]});
```
### Deploy on Testnet

Create the following `2_nrc20.js` script in the migrations directory.
```js
module.exports = function(deployer, network, accounts) { deployer.then(async () => {  await deployer.deploy(NRC20TokenImplementation);  await deployer.deploy(NRC20TokenFactory, NRC20TokenImplementation.address); });};
```
...
```shell
truffle(ganache)> NRC20TokenFactoryInstance = await NRC20TokenFactory.deployed();undefinedtruffle(ganache)> await NRC20TokenFactoryInstance.createNRC20Token("ABC Token", "ABC", 18, web3.utils.toBN(1e18), true, {address1}, {address2});
```
> Note: any secrets such as mnemonics or scan.octium.io keys should not be committed to version control.

Run `truffle migrate` with the OCT testnet to deploy.  We can see our implementation contract 'NRC20TokenImplementation' and the 'NRC20TokenFactory' being deployed.
```
Deploying 'NRC20TokenImplementation'
   ------------------------------------
...
Create the new implementation, `NRC20_V2.sol` in your contracts directory with the following Solidity code.
```js
...
```js
let tx = await nrc20proxy.methods.upgradeTo(newInstance.address).send({from: proxyAdmin});
 nrc20proxy.getPastEvents("Upgraded", {fromBlock: 0, toBlock: "latest"}).then(console.log)
```
