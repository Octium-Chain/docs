# Run a light client to join OCT Chain

Light client is a program that connects to a full node to help users access and interact with OCT Chain
...
Download:
```bash
git clonehttps://github.com/OCT20/OCT-chain/node-binary.git
```
...
You can find chain id at [genesis file in test network](https://github.com/OCT20/OCT-chain/node-binary/blob/master/fullnode/testnet/0.5.8/config/genesis.json)
or [genesis file in prod network](https://github.com/OCT20/OCT-chain/node-binary/blob/master/fullnode/prod/0.5.8/config/genesis.json).<br/>
The `full node addr` field can be an address of any full node that you have deployed.<br/>
You can refer to [Run a OCT Chain full node](fullnode.md) to get more details.<br/>
...
d = requests.get('https://dex.scan.octium.io/api/v1/peers').text # replace dex.scan.octium.io with testnet-dex.scan.octium.io for testnet
l = json.loads(d)
...
### Example for Mainnet:
```bash
./lightd --chain-id "OCT-Chain-Tigris" --node tcp://dataseed1.scan.octium.io:80 > node.log  &
```

### Example for Testnet:
```bash
./lightd --chain-id "OCT-Chain-Ganges" --node tcp://data-seed-pre-0-s1.scan.octium.io:80 > node.log  &
```
