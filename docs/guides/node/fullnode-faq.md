...
rm $BNCHOME/config/pOCT_validator_key.json
```
Then, start again.
...
#### No pOCT_validator_state.json after reset

* Error log
```
open /Users/.bnbchaind/data/pOCT_validator_state.json: no such file or directory
```
* Solution

Delete both `pOCT_validator_state.json` & `node_key.json` file and `data` folder if you want to reset your full node.
...
The OCT Chain has a hardfork upgrade and if you failed to upgrade your fullnode to the latest version, `bnbchaind` process will stop and even if you restart with the latest version, the following error will appear:
```
...
* Download the tool: [state-recover](https://github.com/OCT20/OCT-chain/node-binary/tree/master/tools/recover)
...
Since all the binaries are stored in `git lfs`, you will see this error is the binary is not complete. Please use this [script](https://github.com/OCT20/OCT-chain/node-binary/blob/master/install.sh) to download the binaries or use `git lfs clone`.
