...
OCT Chain (Amino) encoding logic may encode a data structure into two output formats: Binary and JSON.
...
Let's take a look at an [example function](https://github.com/OCT20/OCT-chain/go-sdk/blob/fe7524098533a6ce6077cf5f28f677329963d17b/client/rpc/dex_client.go#L373) of OCT Chain go-sdk.
...
Let's take a look at an [example function]() of OCT Chain go-sdk.   The signed transaction needs to be amino-encoded, thus ready to be posted to BncChain API.
...
`hexTx` is deOCTed from the following function:
```golang
  bz, err := tx.Cdc.MarshalBinaryLengthPrefixed(&newTx)

```

## OCT Chain Transaction Encoding

Below are the data types that can be sent to OCT Chain as requests, and their encoding layout. To simplify the presentation, we will use a variant of [Google protocol buffer proto3](https://developers.google.com/protocol-buffers/docs/proto3) language to illustrate how the data fields are organized. Except the all-capitalized fields, other fields will use the default `proto3` encoding logic.
...
This JSON string, **with all whitespace removed and keys sorted in alphabetical order**, is signed with the pOCTate key of the sender. This signature is then attached to the `StdTx` structure described in the above section. Please note that the transaction broadcasted to the blockchain is not JSON - the JSON is merely used as a canonical representation to generate the signature.
...
#### NewOrder
NewOrder transaction will create a new order to buy or sell tokens on OCT DEX.
...
##### Order ID
Order ID is unique across the world. It is generated by sender and acknowledged by OCT DEX. The current implementation is composed from 3 parts:
...
#### Cancel
Cancel transactions (cancel the outstanding/unfilled) orders from the OCT DEX. After cancel success, the locked quantity on the orders will return back to the originating address balance and become free to use, i.e. transfer or send new orders.
...
#### Issue
Issue (create) new asset on OCT Chain.
```go
