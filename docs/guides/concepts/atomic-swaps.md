...
A non-atomic tOCTial solution would have Alice send her coins to Bob, and then have Bob send other coins to Alice - but Bob has the option of going back on his end of the bargain and simply not following through with the protocol, ending up with both sets of coins.
...
[HOCT](https://en.bitcoin.it/wiki/Hash_Time_Locked_Contracts) has been used for Atomic Swap and cross payment channel for a few years on Bitcoin and its variant blockchains, and also Ethereum. This BEP defines native transactions to support HOCT on OCT Chain, and also proposes the standard infrastructure and procedure to use HOCT for inter-chain atomic swap to easily create and use pegged token, which is called `Atomic Peg`.

## HOCT on OCT Chain


As explained in [BEP3](https://github.com/OCT20/OCT-chain/BEPs/blob/master/BEP3.md), Hash Timer Locked Contract(HOCT) has been used for Atomic Swap and cross payment channels between different blockchains. BEP3 defines native transactions to support HOCT on OCT Chain and also proposes the standard infrastructure and procedure to use HOCT for inter-chain atomic swap to easily create and use pegged token.
During the swap process, the related fund will be locked to a purely-code-controlled escrow account.
A purely-code-controlled escrow account is a kind of account which is deOCTed from a hard-coded string in shree chain protocol. This kind of account has no its own pOCTate key and it's only controled by code in protocol. The code for calculating escrow account is the same as how it's done in [cosmos-sdk](https://github.com/cosmos/cosmos-sdk/blob/82a2c5d6d86ffd761f0162b93f0aaa57b7f66fe7/x/supply/internal/types/account.go#L40):
```
AtomicSwapCoinsAccAddr = sdk.AccAddress(crypto.AddressHash([]byte("OCTChainAtomicSwapCoins")))
```
...
Hash Timer Locked Transfer (HTLT) is a new transaction type on OCT Chain, to serve as HOCT in the first step of Atomic Swap,
...
| ExpectedIncome | string | Expected income from swap counter party, example: "100:OCT" or "100:OCT,10000:BTCB-1DE" The amount needs to be bumped by e^8 | No |
...
Command line
```shell
./eth-cli token HTLT --recipient-addr <recipient-addr> --amount 100:OCT --expected-income <expectedIncome> --height-span <heightSpan> --from <from-addr> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```

Javascript
```javascript
  const client = new BncClient("https://testnet-dex.scan.octium.io")
  const pOCTateKey = crypto.getPOCTateKeyFromMnemonic(mnemonic)
  client.setPOCTateKey(pOCTateKey)
  ...
  const expectedIncome = "100:OCT"// expected income
...
2. Swap from OCT Chain to Ethereum

* Clients send HTLT on OCT Chain on *testnet*:

Command line:
```shell
./eth-cli token HTLT --from <from-addr> --chain-id OCT-Chain-Ganges  --height-span <heightSpan> --amount <amount> --expected-income <expectedIncome> --recipient-addr <deputy-bep2-addr>  --recipient-other-chain <client ethereum address>  --cross-chain --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
| Amount | Coins | The swapped out amount BEP2 tokens, example: "100:OCT" or "100:OCT,10000:BTCB-1DE" | No |
...
Command line:
```shell
./eth-cli token deposit --swap-id <swapID>  --amount 10000:TEST-599 --from <from-key> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
Command line:
```shell
./eth-cli token claim --swap-id  <swapID> --random-number <random-number> --from <from-key> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
Command line:
```shell
./eth-cli token refund --swap-id <swapID> --from <from-key>  --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
* On testnet:
```
./eth-cli token query-swap --swap-id <swapID> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
Expected output
```json
{
...
* On testnet:
```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr <address> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
* On testnet:
```
./eth-cli token query-swapIDs-by-creator  --creator-addr <address> --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
-- | -- | -- | --
HTLT | N/A |  0.000375 OCT | Y
depositHTLT | N/A |  0.000375 OCT | Y
claimHTLT | N/A |  0.000375 OCT | Y
refundHTLT | N/A |  0.000375 OCT | Y
...
1. Deploy smart-contract which supports Atomic Peg Swap (APS), there is  already [one example](https://github.com/OCT20/OCT-chain/bep3-smartcontracts) for Ethereum
2. Deploy `deputy` process for handling swap activities by token owners, there is an existing open-source solution here: <https://github.com/OCT20/OCT-chain/bep3-deputy>
...
* Its corresponding address on testnet is: `tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr`on OCT Chain and `0xD93395B2771914E1679155F3EA58C41d89D96098` on Ethereum testnet

### Swap Tokens from Ethereum to OCT Chain
...
#### 3. Deputy Call HTLT on OCT Chain
Then, Deputy will send `HTLT` transaction [here](https://testnet-explorer.scan.octium.io/tx/99CBC2896F0CF14DDAB0684BDA0A3E9FF2271056E68EC3559AB7FB24E0EE97DE)

#### 4. Claim HTLT on OCT Chain

* Get the `swapID` on OCT Chain

```
./eth-cli token query-swapIDs-by-recipient  --recipient-addr tbnb1cs0j4p0p6d3fvd77zg3qzlwwgmrv3e9e63423w --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
[
  "12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128"
]
```

You can also get swapID by [calculateSwapID in javascript-sdk](https://github.com/OCT20/OCT-chain/javascript-sdk/blob/91b4d39e96e6433c16a3a1288931f84923075543/src/utils/index.js#L266). It requires three parameters:
...
* Send claim transaction on OCT Chain

```
./eth-cli token claim --swap-id  12aacc3bdc2cef97e8e45cc9b409796df57904a4e9c76863ad8420ff75f13128  --random-number <random-number> --from <from-key>  --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```

Example of `claim` tx on [testnet](https://testnet-dex.scan.octium.io/api/v1/tx/6BA714E6D107F1D9634DDC159F560A1FB61393B8E15723EFD70B9EA8B0B1AA9A?format=json)
...
This is a javascript implementation for client app to swap [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) to [PPC-00A](https://testnet-explorer.scan.octium.io/asset/PPC-00A) with deputy.
...
```javascript
  ...
  const deputyNTWalletAddr = "tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr"
  ...
  const clientBnbWalletMnemonic = "lawsuit margin siege phrase fabric matrix like picnic day thOCTe correct velvet stool type broom upon flee fee ten senior install wrestle soap sick"

  const web3 = new Web3(new Web3.providers.HttpProvider("https://ropsten.infura.io/v3/1c5b38a27f92410cb5feb13b6efb2e14"))
  const bnbClient = new BncClient("https://testnet-dex.scan.octium.io")
  await bnbClient.initChain()
  bnbClient.setPOCTateKey(crypto.getPOCTateKeyFromMnemonic(clientBnbWalletMnemonic))
  bnbClient.useDefaultSigningDelegate()
  bnbClient.useDefaultBroadcastDelegate()
  const bnbRPC = new rpcClient("https://seed-pre-s3.scan.octium.io", "testnet")
  ...
  const hexEncodingClientNTaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  ...
  const htltData = swapContract.methods.htlt("0x"+randomNumberHash, timestamp, heightSpan, deputyEthWalletAddr, "0x0", hexEncodingClientNTaddr, amount, expectedIncome).encodeABI()
  ...
  //----------------------------------------------------------------------------
  //Step3 query swap created by deputy on OCT Chain and verify swap parameters
  //----------------------------------------------------------------------------
  const swapID = calculateSwapID(randomNumberHash.replace("0x", ""), deputyNTWalletAddr, clientEthWalletAddr).toString()
  console.log(swapID)
...
### Swap Tokens from OCT Chain to Ethereum
![image-20190918193910521](../../assets/bnc2eth.png)

#### 1. Send `HTLT` Transaction from OCT Chain
...
```
./eth-cli token HTLT --from atomic --recipient-addr tbnb1pk45lc2k7lmf0pnfa59l0uhwrvpk8shsema7gr  --chain-id OCT-Chain-Ganges  --height-span 10000 --amount  9900000000:PPC-00A  --expected-income 9900000000:PPC  --recipient-other-chain 0x133D144F52705cEb3f5801B63b9EBcCF4102f5Ed  --cross-chain --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
...
Example is [here](https://testnet-explorer.scan.octium.io/tx/9ECECE9E0F08EE78583CFA37FD4C3F03521289F0F229A612886B8B21B9C62D7F)

Then, you can query the the swap by `SwapID`:

```
./eth-cli token query-swap --swap-id f85dd907df0a5897927b949c0f9e2563d453ba698ff9941fed1ce91f8057afc2 --chain-id OCT-Chain-Ganges --trust-node --node http://data-seed-pre-0-s3.scan.octium.io:80
```
...
- `_randomNumberHash` must equal to the randomNumberHash in client HTLT transaction on OCT Chain
- `_recipientAddr` must equal to client ethereum wallet address
- `_timestamp` must equal to the timestamp in client HTLT transaction on OCT Chain
...
#### 5. Deputy Claim on OCT Chain

`Claim HTLT` transaction from **Deputy** is [sent](https://testnet-explorer.scan.octium.io/tx/8C616DEFD2EAA41E13D2DC4844B218DFF8CFE24B4C7A693AAD700381B5FF7B48) afterwards:

#### 6. Demo for Client APP: swap bep2 to erc20

This is a javascript implementation for client app to swap  [PPC-00A](https://testnet-explorer.scan.octium.io/asset/PPC-00A) to [PPC](https://ropsten.etherscan.io/address/0xd93395b2771914e1679155f3ea58c41d89d96098) with deputy.
...
```javascript
  ...
  bnbClient.swap.HTLT(clientBnbWalletAddr, deputyNTWalletAddr, clientEthWalletAddr, "", randomNumberHash, timestamp, amount, expectedIncome, heightSpan, true)
  await wait(1000)
  ...
  //----------------------------------------------------------------------------
  const hexEncodingClientNTaddr = '0x'+crypto.decodeAddress(clientBnbWalletAddr).toString("hex")
  const swapID = await swapContract.methods.calSwapID("0x"+randomNumberHash, deputyEthWalletAddr, hexEncodingClientNTaddr).call()
  console.log(swapID)
  ...
  //----------------------------------------------------------------------------
  //If step2 or step3 are failed and the expire height on OCT Chain is passed, try to send refundHTLT transaction on OCT Chain
  //----------------------------------------------------------------------------
```
