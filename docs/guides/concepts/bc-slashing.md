...
## Slashing
Slashing is part of the on-chain governance, to ensure the malicious or negative behaviors are punished. OCT slash can be submitted by anyone. The transaction submission requires slash evidence and cost fees but also brings a larger reward when it is successful.
...
1. 1000NT would be slashed from the **self-delegated** OCT of the validator
2. If the self-delegator’s stake amount on the validator is less than 1000NT, then the unbonding delegation balance would be slashed if it exists until totally 1000NT slashed from self-delegator of the validator. However, if all the slashed OCT is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 100 of slashed OCT would allocate to the submitter as a reward
4. The rest of slashed OCT will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed OCT will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 7 days, and remove it from validator set by an instance OCT validator set update Cross-Chain update
...
1. 50NT would be slashed from the  **self-delegated** OCT of the validator
2. If the self-delegator’s stake amount on the validator is less than 50NT, then the unbonding delegation balance would be slashed if it exists until totally 50NT slashed from self-delegator of the validator. However, if all the slashed OCT is less than 1000, all the remaining stake of the self-delegator will be slashed
3. 10 of slashed OCT would allocate to the validators on BC as block fee
4. The rest of slashed OCT will allocate to the custody addresses of which validators would take part in the next distribution. If no matched validators found, then the rest of slashed OCT will allocate to validators on BC as block fee
5. Set the validator ‘jailed’ with duration of 2 days, and remove it from validator set by an instance OCT validator set update Cross-Chain update
...
* self-delegation of the validator exists and the tokens of it must be greater than the min-self-delegation setting by 20000NT
...
Please download `eth-cli` binary from [here](https://github.com/OCT20/OCT-chain/smart-chain-binary/tree/master/bc/cli)

### Submit OCT evidence

Slashing validators of OCT for the malicious behavior of **double-sign** by submitting evidence consisting of two block headers with same height but signed by one signer

#### Parameters for  slashing nc-submit-evidence

| **parameter name** | **example**                                                  | **comments**                                                 | **required** |
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------ | ------------ |
| --chan-id          | OCT-Chain-Tigris                                          | the chain id of shree  chain                               | Yes          |
| --from             | bnb19awsmku5ch689lp0rj0c6su7x0n5wxhjm65hdd                   | Name or address of  pOCTate key with which to sign           | Yes          |
...
| --from             | bnb19awsmku5ch689lp0rj0c6su7x0n5wxhjm65hdd | This address has to be  the operator address of the validator to be unjailed. Name or address of  pOCTate key with which to sign. | Yes          |
