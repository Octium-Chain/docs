...
1. Compose a data structure. please note `msgs`, `memo`, `source`, `data` are the same as in the above `payload`.
    ...
    - `chain_id`: a string, unique ID for the Chain, it stays the same for most time, but may vary as OCT Chain evolves;
 ...
 Here is an example in [go-sdk](https://github.com/OCT20/OCT-chain/go-sdk/blob/master/types/tx/stdsign.go#L22):
 ```golang
 // StdSignMsg def
...
The `signature` will be encoded together with transaction message and sent as `payload` to OCT Chain node via RPC or http REST API, as described in the above section.
