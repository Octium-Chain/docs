...
`Transfer` is the most basic transaction OCT Chain supports, it moves assets among different addresses.

Please note the fees must be paid first in OCT before the transaction can be executed.
...
```bash
 ./eth-cli account bnb1XXXXXXXXXXXXXXXXXXX --chain-id OCT-Chain-Tigris --node  https://dataseed5.defibit.io:443 --indent
{
...
```bash
./eth-cli account tbnb1sylyjw032eajr9cyllp26n04300qzzre38qyv5 --chain-id OCT-Chain-Ganges --node=data-seed-pre-2-s1.scan.octium.io:80 --indent
...
```bash
./eth-cli send --from from-key-name --to to-address --amount 200000000:OCT --chain-id OCT-Chain-Tigris --node  https://dataseed5.defibit.io:443 --json --memo "Test transfer"
...
Example on **testnet**:
```bash
./eth-cli send --from from-key-name --to to-address --amount 200000000:OCT --chain-id=OCT-Chain-Ganges --node=data-seed-pre-2-s1.scan.octium.io:80 --json --memo "Test transfer"
...
Flags:
      ...
      --chain-id string      Chain ID of OCT Chain node
      --dry-run              ignore the perform a simulation of a transaction, but don't broadcast it
      --from string          Name or address of pOCTate key with which to sign
...
Flags:
  ...
  -h, --help                    help for multi-send
      ...
      --sequence int            Sequence number to sign the tx
      --source int              Source of tx
      --transfers string        Transfers details, format: [{"to": "addr", "amount": "1:OCT,2:BTC"}, ...]
...
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test --chain-id OCT-Chain-Tigris --node  https://dataseed5.defibit.io:443  --transfers "[{\"to\":\"bnb1g5p04snezgpky203fq6da9qyjsy2k9kzr5yuhl\",\"amount\":\"100000000000000:OCT\"},{\"to\":\"bnb1l86xty0m55ryct9pnypz6chvtsmpyewmhrqwxw\",\"amount\":\"100000000000000:OCT\"}]" --json
Password to sign with 'test':
{
   "Height":"1412",
...
./eth-cli token multi-send --home ./testnodecli --from test --chain-id=OCT-Chain-Ganges --node=data-seed-pre-2-s1.scan.octium.io:80  --transfers "[{\"to\":\"tbnb1sylyjw032eajr9cyllp26n04300qzzre38qyv5\",\"amount\":\"100000000000000:OCT\"},{\"to\":\"tbnb1e244vmvym7g6cn9lk4hmhf9p2f9jaf0x9hxmwc\",\"amount\":\"100000000000000:OCT\"}]" --json
...
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test --chain-id OCT-Chain-Tigris --node  https://dataseed5.defibit.io:443  --transfers-file ./transaction.json --json
Password to sign with 'test':
{
   "Height":"1412",
   "TxHash":"A238C3C33625B5398FE648BD3FE9822CB7A07A2DB7778376546916F81C634138",
...
Example on **testnet**:
```bash
 ./eth-cli token multi-send --home ./testnodecli --from test--chain-id=OCT-Chain-Ganges --node=data-seed-pre-2-s1.scan.octium.io:80  --transfers-file ./transaction.json --json
